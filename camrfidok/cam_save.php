...
if (isset($_POST['rfid']) && $_POST['imgBase64']) {
    ...
    /// mengambil data siswa.id, kelas_id, rfid_unique by
    $sql = "SELECT `siswa`.`kelas_id`, `siswa`.`siswa_id`, `siswa`.`siswa_rfid` 
            FROM `siswa` 
            WHERE `siswa`.`siswa_rfid` = '$rfid';";
    ...
    if ($result->num_rows > 0) {
        ...
        $sql = "SELECT * FROM `jadwal` WHERE `kelas_id` = '$kelas_id' AND `jadwal_hari` = '$hari'";
        $result = $koneksi->query($sql);
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
            ...
            $sql = "SELECT * FROM `rekap` WHERE `jadwal_id` = '$jadwal_id' AND `siswa_id` = '$siswa_id'";
            $result = $koneksi->query($sql);
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                ...
                if (strtotime($jam_sekarang) <= strtotime($jadwal_pulang)) {
                    ...
                    $folderPath = 'rekapfoto';
                    if (!file_exists($folderPath)) {
                        mkdir($folderPath);
                    }

                    $file = @fopen('./rekapfoto/'.$randomName, "w");
                    if ($file != false){
                ...
                if (strtotime($jam_sekarang) >= strtotime($jam_jangkawaktu) && strtotime($jam_sekarang) <= strtotime($jadwal_masuk)) {
                    ...
                    // Hadir
                    $sql = "INSERT INTO `rekap` (`rekap_id`, `jadwal_id`, `siswa_id`, `rekap_tanggal`, `rekap_masuk`, `rekap_keluar`, `rekap_photomasuk`, `rekap_photokeluar`, `rekap_keterangan`) 
                            VALUES (NULL, '$jadwal_id', '$siswa_id', '$tanggal_sekarang', '$jam_sekarang', NULL, '$randomName', NULL, 'Hadir Masuk');";
                    if ($koneksi->query($sql) === TRUE) {
                        die(json_encode(array(
                            'status' => true, 
                    ...
                    // Hadir Terlambat
                    $sql = "INSERT INTO `rekap` (`rekap_id`, `jadwal_id`, `siswa_id`, `rekap_tanggal`, `rekap_masuk`, `rekap_keluar`, `rekap_photomasuk`, `rekap_photokeluar`, `rekap_keterangan`) 
                            VALUES (NULL, '$jadwal_id', '$siswa_id', '$tanggal_sekarang', '$jam_sekarang', NULL, '$randomName', NULL, 'Hadir Terlambat');";
                    if ($koneksi->query($sql) === TRUE) {
                        die(json_encode(array(
                            'status' => true, 
                            'message' => 'Berhasil Absen Masuk'
